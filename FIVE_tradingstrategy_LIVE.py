...
#
# THIS IS THE LIVE MODEL BASED ON BACKTESTING RESULTS
#
import sys
...
class tradingstrategy(tpqoa):
    ...
    def __init__(self, conf_file, instrument, stopmin):
        ...
        self.stoptime = self.starttime + timedelta(minutes = stopmin)
    ...
    def stream_data(self):
        ...
        print('trading will stop at %12s' % self.stoptime)
        print(20* '-')
        logging.info(20 * '-')
        logging.info('trading starting at %12s' % self.starttime) 
        logging.info('trading will stop at %12s' % self.stoptime) 
        logging.info(20 * '-')
        self.ticks = 0
        ...
        for msg_type, msg in response.parts():
            if msg_type == 'pricing.Price':
                ...
                if datetime.datetime.now() >= self.stoptime:
                    self.close_out()
                    break
    ...
    def on_success(self, time, bid, ask):
        ...
        if len(self.dataresam) > self.mom2: 
